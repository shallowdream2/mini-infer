...
| 阶段类型 | 异常触发条件 | 说明 |
|---------|------------|------|
| 性能优化阶段（Phase 3/6/12 等） | 与 HF baseline 相比低于 50%，或相比上一阶段**同一 workload** 倒退超过 10% | 以 HF baseline 或上一阶段同 workload 数据为基线 |
| 调度/功能阶段（Phase 7/9/10 等） | 新 workload 的核心指标无改善，或原有 workload 吞吐倒退超过 10% | 核心指标由本阶段验收标准定义（如 ITL spike、hit rate），不强求吞吐提升 |
| 阶段内多次运行 | 同一配置两次结果差异 > 20% | 测量不稳定，需检查 warmup 和环境噪声 |

如果 benchmark 结果明显不合理（触发以上任一条件），**不得继续运行或得出结论**，立即执行：

"""
Phase 4 TP=2 测量引擎（HF Pipeline Parallel）。
...
- 真 TP：每层按 head 维度切分，层内通信是 all-reduce
...
class _StubTPTokenizer:
    ...
    def batch_decode(self, token_ids: torch.Tensor, skip_special_tokens: bool = True) -> list[str]:
        return [f"[tp-stub-{i}]" for i in range(token_ids.shape[0])]
...
class TPEngine:
    ...
    def __init__(self, config: EngineConfig) -> None:
        ...
        if config.dry_run:
            self.tokenizer: _StubTPTokenizer | "AutoTokenizer" = _StubTPTokenizer()
            self.model = None
        ...
        if torch.cuda.device_count() < 2:
            raise RuntimeError(
                f"TPEngine 需要至少 2 块 GPU，当前检测到 {torch.cuda.device_count()} 块。"
            )
        ...
        print(f"TPEngine: 使用 device_map='balanced' 加载模型到 2 块 GPU ...")
        ...
        print(f"TPEngine: embedding 层在 {self._first_device}，hf_device_map 片段：")
    ...
    def generate(self, prompts: list[str], max_new_tokens: int = 128) -> list[str]:
        ...
        if self.config.dry_run:
            return [f"[tp-stub:{p[:8]}]" for p in prompts]

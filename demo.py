"""demo.py — mini-infer 功能对比演示

同一个 prompt，不同配置，并排展示文本输出 + 关键指标。
...
用法示例：
  ...
  python demo.py --model $MODEL --mode cuda-graph
  python demo.py --model $MODEL --mode prefix-cache
  python demo.py --model $MODEL --mode all --prompt "What is machine learning?"
"""
...
def _run_prompts(
    ...
    """运行 prompts，返回 (texts, decode_tps, weight_mb)。"""
    ...
    texts = [runner.tokenizer.decode(s.generated_token_ids, skip_special_tokens=True) for s in states]
...
def demo_quant(model_path: str, device: str, prompts: list[str], max_new_tokens: int) -> None:
    ...
    for i, prompt in enumerate(prompts):
        ...
        max_lines = max(len(fp16_lines), len(w8a8_lines))
    ...
    print()
    print(f"  显存节省 {(1-mem_ratio)*100:.1f}%  |  W8A8 decode {'慢' if tps_ratio < 1 else '快'} {abs(1-tps_ratio)*100:.0f}%（mixed fallback，小 M 场景预期慢于 FP16）")
...
def demo_cuda_graph(model_path: str, device: str, prompts: list[str], max_new_tokens: int) -> None:
    ...
    print(f"模型路径 : {model_path}")
    print(f"说明     : CUDA Graph 对 decode_batch 做静态图捕获，消除 Python dispatch 开销")
    ...
    for use_graph, label in ((False, "eager"), (True, "cuda_graph")):
        ...
        texts, tps, mem = _run_prompts(runner, kv, prompts, max_new_tokens)
        results[label] = {"texts": texts, "tps": tps, "mem": mem}
        print(f"  Decode: {tps:.0f} tok/s")
        del runner, kv
        _free_gpu()

    # ── 正确性验证 ────────────────────────────────────────────────
    _section("正确性验证（Eager vs CUDA Graph 输出应完全相同）")
    all_match = True
    ...
    print(f"  CUDA Graph {direction} {abs(ratio-1)*100:.1f}%  |  输出一致性：{'✓ 全部一致' if all_match else '✗ 有差异'}")
    print(f"  注：bs=1 小 batch 提升最明显；batch 越大、模型越大效果越显著")
...
def demo_prefix_cache(model_path: str, device: str, max_new_tokens: int) -> None:
    ...
    print(f"模型路径 : {model_path}")
    print(f"说明     : 第一次请求写入 prefix cache，第二次相同前缀直接命中，TTFT 明显缩短")
    ...
    shared_prefix = (
        ...
        "well-structured answers. When explaining technical concepts, you use "
        "concrete examples and highlight the key intuitions. "
    )
    suffix_a = "Please explain what a neural network is."
    suffix_b = "Please explain what gradient descent is."
    ...
    _sync()
    t0 = time.perf_counter()
    out_a1 = engine.generate([prompt_a], max_new_tokens=max_new_tokens)[0]
    _sync()
    ttft_cold = (time.perf_counter() - t0) * 1000
    print(f"  TTFT（冷启动）: {ttft_cold:.1f} ms")
    print(f"  输出: {out_a1[:80]}...")
    ...
    print(f"  {'阶段':<20} {'TTFT (ms)':>12}")
    print(f"  {'-'*34}")
    print(f"  {'冷启动（miss）':<20} {ttft_cold:>12.1f}")
    print(f"  {'命中（hit）':<20} {ttft_warm:>12.1f}")
    print()
    print(f"  前缀命中 TTFT 减少 {reduction:.1f}%")
    print(f"  注：e2e 时间包含调度器开销；纯 prefill 节省在 TTFT 指标里体现")
...
def main() -> int:
    ...
    parser.add_argument("--prompt", type=str, default="", help="自定义 prompt（单条；留空使用内置演示 prompt）")
    parser.add_argument("--max-new-tokens", type=int, default=64, help="每条 prompt 最多生成 token 数（默认 64）")
    parser.add_argument("--device", type=str, default="cuda:0", help="推理设备（默认 cuda:0）")
    args = parser.parse_args()

    prompts = [args.prompt] if args.prompt else DEFAULT_PROMPTS[:2]
